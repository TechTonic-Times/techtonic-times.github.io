...
async function getArticleData() {
  ...
  try {
    ...
    _articleDataCache = null; // Reset so it can retry on next call
    throw err;
  }
}
...
window.addEventListener('scroll', () => {
  if (!isScrolling) {
      window.requestAnimationFrame(() => {
          ...
          toggleBackToTop();
          checkVisibility(); // Moved here from separate listener
          isScrolling = false;
      });
      isScrolling = true;
  }
...
async function loadRelatedArticles() {
  ...
  try {
      ...
      // Re-observe new images for blur effect
      initImageObserver();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const loadNonCriticalContent = () => {
      ...
      if (document.getElementById('dynamic-link-container')) {
          // Wait slightly for related articles to populate DOM
